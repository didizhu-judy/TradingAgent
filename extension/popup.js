...
function renderPositions(list) {
  if (!list || list.length === 0) {
    el.positionsList.innerHTML = '<div class="position">暂无持仓</div>';
    return;
  }
  el.positionsList.innerHTML = list.map(function(p) {
    ...
    return '<div class="position"><div class="name">' + name + '</div><div class="ticker">' + ticker + '</div><div class="row"><span>Qty ' + qty + '</span><span>Price ' + curr + '</span></div><div class="row"><span>Avg ' + cost + '</span><span class="' + plCls + '">' + plStr + '</span></div></div>';
  }).join('');
}
